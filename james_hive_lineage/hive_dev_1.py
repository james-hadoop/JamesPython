...
def remove_partition_part(sql):
    sql = re.sub(r"(partition\(\w*=\w*\))", " ", sql)

    return sql

# 删除partion子句
def remove_overwrite_table(sql):
    sql = re.sub(r"OVERWRITE\s*TABLE", " INTO ", sql, flag=re.I)
